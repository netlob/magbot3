 ...
 * The four functions accessed are
 ...
 * sortReminders - sorts google calendar reminders on method and minute.
 */
module.exports = {
...
function google2mag(gog) {
    return {
        description: gog.summary?event.summary:'Geen titel',
        start: new Date(gog.start.dateTime),
        end: new Date(gog.start.dateTime),
        fullDay: false,
        location: gog.location?gog.location:false,
        content: gog.description?gog.description:false,
        type: 1
    }
    // m.createAppointment(options)
    // .then(info => { console.dir(info) })
}
