...
module.exports = shelf.model('users', {
    ...
    login: async function(oAuth, mAuth) {
        ...
        if (this.magister.profileInfo.isParent) {
            ...
            this.log.debug(`User is parent with ${children.length} children.`);
        }
        // Login to google.
        ...
        this.google.on('tokens', (tokens) => {
            this.log.debug(`Updating accesstoken...`);
        ...
        await this.update();
        this.log.debug(`Logged in successful.`);
        return this;
    },
    ...
    setupCalendars: async function(doRenew = false) {
        ...
        do { // Setup the calendars. For children this is run once.
            ...
            this.log.debug(`Updating ${childId}'s calendars...`, localCalendars);
    ...
    registerUpdate: async function(oAuth, mAuth, params) {
        ...
        for (let calendarType in params.calendarTypes) {
            options.calendarTypes[calendarType].defaultReminders[0].minutes = (params.reminderMin || 5);
        }
        let specialReminders = [options.calendarTypes[calendarType].defaultReminders[0]];
        if (params.specialEmailReminder) specialReminders.concat({method: 'email', minutes: 10080});
        if (params.specialDayReminder) specialReminders.concat({method: 'popup', minutes: 1440});
        options.calendarType.special.defaultReminders = specialReminders;
