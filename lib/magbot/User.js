...
module.exports = shelf.model('users', {
    ...
    setupCalendars: async function(doRenew = false) {
        ...
        // Get calendars the user has & check if they are present.
        ...
        do { // Setup the calendars. For children this is run once.
            ...
            for (let calendarType in setupCalendars) {
                ...
                // If the user deleted the calendar, remove it everywhere.
                    ...
                    //delete localCalendars[calendarType]; //TODO Reintroduce this after many tests.
                    this.log.debug(`WOULD HAVE Removed magbot calendar ${calendarType}`, {calendarId});
                }
            }
        ...
        do {
            ...
            for (let mApp of mApps) {
                try {
                    ...
                    gApps = await this.updateMagisterAppointment(dCalType, localCalendars, gApps, mApp, false);
                    gApps = await this.updateMagisterAppointment('full', localCalendars, gApps, mApp, false);
                } catch(err) {
                    this.log.warn(`Error on appointment update, retrying with patch...`, {err: err.toString()});
                    try {
                        let dCalType = Sync.mag2caltype(mApp); // Determined calendartype.
                        gApps = await this.updateMagisterAppointment(dCalType, localCalendars, gApps, mApp, true);
                        gApps = await this.updateMagisterAppointment('full', localCalendars, gApps, mApp, true);
                        this.log.warn(`Retry was successful actually! :F`);
                    } catch(err) {
                        this.log.error(`Retry failed aswell, skipping appointment! :(`, {err: err.toString()});
                    }
                }
            }
                ...
                for (let gAppId in gCal) {
                    ...
                    if (gAppId.includes('magbot') && gApp.status !== 'cancelled') {
                        try {
                            ...
                            await sleep(Math.floor(Math.random() * 1000)); //Sleep a bit to ease google.
                            this.log.debug(`Deleted ${gAppId}.`);
                        } catch(err) {
                            this.log.error(`Failed deleting appointment!? Skipping... :(`, {err: err.toString()});
                        }
                    } else if(!gAppId.includes('magbot') { // or is it else if()?
						const mApp = Sync.google2mag(gApp); // transvorms google appointment into magister appointment
					    this.magister.createAppointment(mApp) // is this the correct way to use the magister class?
                        .then(info => { console.dir(info) }) // for debug
					}
                }
            }
        } while (this.nextChild());
     ...
     * @param {boolean} forcedUpdate - Wether or not to force update instead of insert.
     * @returns gApps, with the updated appointment removed for purging purposes.
     */
    updateMagisterAppointment: async function(caltype, localCalendars, gApps, mApp, forcedUpdate) {
        if (caltype in localCalendars) {
            ...
            if (forcedUpdate || currentApp) {
                // If we should patch, patch otherwise just skip this appointment.
                if (forcedUpdate || Sync.shouldChange(currentApp, app)) {
    ...
    magisterAppointments: async function() {
        return await this.magister.appointments(
            new Date(), new Date(Date.now() + this.getSecondsToCheck())
        );
    },
     ...
     */
    calendarAppointments: async function(calendarId, showDeleted = true) {
        return (await this.calendar.events.list({
			auth: this.google,
            calendarId: calendarId,
            timeMin: new Date(new Date().setHours(0,0,0,0)),
            timeMax: new Date(new Date(Date.now() + this.getSecondsToCheck())
    ...
    /**
     * Gets the seconds for users to check.
     * Primes get a bit more checked weeks.
     */
    getSecondsToCheck: function() {
        return this.isPrime() ? 48384e5 : 12096e5;
    },

    /**
     * Checks if user is prime.
     * We will give him some perks :)
     */
    isPrime: function() {
        return this.get('donations') >= 5;
    }
    ...
    registerUpdate: async function(oAuth, mAuth, params) {
        ...
        let setupcalendars = {};
        if (params.fullcalendar == 'true') setupcalendars.full = '';
        if (params.splitcalendars == 'true' || params.fullcalendar == 'false') {
        ...
        options.simpleSummary = (params.simplesummary == 'true') || false;
        options.simpleShowTeacher = (params.simpleshowteacher == 'true') || false;
        options.showSchoolHour = (params.showschoolhour == 'true') || false;
        options.calendarTypes.outages.hidden = !(params.showoutages == 'true');
        for (let calendarType in options.calendarTypes) {
            if (!_.isEmpty(options.calendarTypes[calendarType].defaultReminders)) {
                ...
                options.calendarTypes[calendarType].defaultReminders[0].minutes = (parseFloat(params.remindermin) || 5);
            }
        }
        let specialReminders = [options.calendarTypes.special.defaultReminders[0]];
        if (params.specialemailreminder == 'true') specialReminders.push({method: 'email', minutes: 10080});
        if (params.specialdayreminder == 'true') specialReminders.push({method: 'popup', minutes: 1440});
        options.calendarTypes.special.defaultReminders = specialReminders;
        options.calendarTypes.full.specialColors = (params.specialcolors == 'true') || true;
        wLog.debug(`Set options.`, {email: userinfo.data.email, options: options});
        user.set('options', options);
		await user.update(); // Quick update to better ensure saving of options.
        ...
        // Check for and remove action required message
