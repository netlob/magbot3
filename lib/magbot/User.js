...
module.exports = shelf.model('users', {
    ...
    login: async function(oAuth, mAuth) {
        ...
        // Sync / setup the calendar information.
    ...
    setupCalendars: async function() {
        ...
        do { // Setup the calendars. For children this is run once.
            ...
            let localCalendars = _.defaultsDeep(
                (this.isParent() ? calendars[childId] : calendars),
                this.get('options').setupCalendars);
            for (let calendarType in localCalendars) {
                ...
                // If calendar is set to empty, create one.
                if (calendarId == '') {
                    ...
                    if (this.isParent()) calendars[childId][calendarType] = calendarId;
                ...
                // If calendar is already here, patch it if necessary.
                } else if (calendarId in calsFromGoogle) {
                    if (!_.isEqual(calendarOptions.defaultReminders, calsFromGoogle[calendarId])) {
                        await this.calendar.calendarList.patch({
                            calendarId: calendarId,
                            resource: {
                        ...
                        console.log(`PATCHED REMINDERS OF ${calendarId}!`);
                    }
                // If the user deleted the calendar, remove it in magbot too.
                } else {
                    if (this.isParent()) delete calendars[childId][calendarType];
                    else delete calendars[calendarType];
                }
            }
