		...
		fs.readdirSync(database).forEach(file => {
			fs.readFile('../db/'+file, function read(err, data) {
				...
				.then((all => {
					...
					.then((all => {
						pushCalendar(all)
						// getTokens(all.oauth2Client)
						console.dir(all.oauth2Client.credentials.refresh_token)
						return;
					}))
				}))
			});
...
const getTokens = async function(params) {
	const { tokens } = await oauth2Client.getToken(params.credentials.refresh_token)
	return tokens;
}